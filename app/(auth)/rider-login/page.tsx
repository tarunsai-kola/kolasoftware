...
import { sendRiderPasswordResetOtp } from '@/app/actions/auth'
...
export default function RiderLoginPage() {
  ...
  // Helper to generate synthetic email from phone
  const getSyntheticEmail = (p: string) => {
    const cleanPhone = p.replace(/[^0-9]/g, '')
    return `${cleanPhone}@rider.kolasoftware.app`
  }
  ...
  const handleSendResetOtp = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await sendRiderPasswordResetOtp(phone.trim())
      if (result.error) {
        toast.error(result.error)
        return
      }
      
      // Mask the real email for display (e.g. j***@gmail.com)
      if (result.email) {
        const [username, domain] = result.email.split('@')
        const masked = `${username[0]}***@${domain}`
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.verifyOtp({
      email: syntheticEmail,
      token: otp,
      type: 'magiclink' // Recovery uses magiclink verification endpoint in client
    })
      ...
      <div className="w-full max-w-md space-y-8 rounded-2xl bg-white p-10 shadow-sm border border-gray-100">
        <div>
          <h2 className="mt-6 text-center text-3xl font-extrabold tracking-tight text-gray-900">
            {step === 'login' && 'Rider Login 🛵'}
            {step === 'forgot-password' && 'Reset Password'}
            {step === 'verify-otp' && 'Verify Email'}
            {step === 'new-password' && 'New Password'}
          </h2>
          <p className="mt-2 text-center text-sm text-gray-600">
            {step === 'login' && 'Sign in to view your assigned deliveries'}
            {step === 'forgot-password' && 'Enter your phone number to receive a reset code'}
            {step === 'verify-otp' && `Enter the verification code sent to ${maskedEmail}`}
